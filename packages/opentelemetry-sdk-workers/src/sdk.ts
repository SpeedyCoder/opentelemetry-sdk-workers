...
type NodeSdkConfig = NodeSdkConfigBase &
	(NodeSdkBuiltInExporter | NodeSdkExternalExporter);
...
export class WorkersSDK<TEnv extends Record<string, unknown> = {}> {
	...
	private readonly allowedHeaders: (string | RegExp)[] = [
		"user-agent",
		"cf-ray",
	];
	...
	public static fromEnv<TEnv extends Record<string, unknown> = {}>(
		eventOrRequest: Request | ScheduledEvent,
		env: TEnv,
		ctx: CfContext,
		...
		const resource = new Resource(attributes).merge(
			new Resource({
		...
		const consoleEnabled =
			rawConsoleLogValue === "1" || rawConsoleLogValue === "true";
		return new WorkersSDK<TEnv>(eventOrRequest, ctx, env, {
			...
			logExporter: loggingEnabled
				? OTLPJsonLogExporter.fromEnv(env)
				: undefined,
	...
	public constructor(
		private eventOrRequest: Request | ScheduledEvent,
		private ctx: CfContext,
		envOrConfig: Record<string, unknown> | NodeSdkConfig,
		configOrUnk?: NodeSdkConfig
	) {
		...
		this.requestTracer = this.traceProvider.getTracer(
			"opentelemetry-sdk-workers",
			"0.1.0"
		);
	...
	private async _fetch(
		fetchTarget: { fetch: Fetcher["fetch"] },
		request: Request | string,
		requestInitr?: RequestInit | Request
	): Promise<Response> {
	...
	public async end() {
		try {
			...
			console.error(
				"Make sure that your endpoint and/or exporters are correctly configured."
			);
		}
	}
	...
	private initSpan(attributes: Attributes, initFromIncomingRequest: boolean) {
		...
		const span = this.requestTracer.startSpan(
			name,
			{
				...
				kind:
					"type" in this.eventOrRequest ? SpanKind.INTERNAL : SpanKind.SERVER,
				startTime: Date.now(),
			},
			context
		);
		...
		if ("type" in this.eventOrRequest) {
			...
			span.setAttributes({
				...
				[SemanticAttributes.HTTP_SCHEME]: url.protocol.substring(
					0,
			...
			for (const headerKey of request.headers.keys()) {
				...
				span.setAttribute(`http.request.header.${headerKey.toLowerCase()}`, [
